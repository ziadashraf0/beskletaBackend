...
router.post("/signup", async (req, res) => {
  ...
  var clientAge =
    parseInt(date.format(new Date(), "YYYY")) -
    parseInt(date.format(birthDate, "YYYY"));

  if (
    parseInt(date.format(new Date(), "MM")) <
    parseInt(date.format(birthDate, "MM"))
  )
    ...
    if (
      parseInt(date.format(new Date(), "DD")) <
      parseInt(date.format(birthDate, "DD"))
    )
      clientAge = clientAge - 1;
  }
...
router.post("/viewProfile", async (req, res) => {
  ...
  const client = await Client.findOne({ userName: req.body.userName }).select({
    password: 0
  });
...
router.put("/activateAccount", async (req, res) => {
  ...
  if (
    !req.body.cardValidityDate ||
    !req.body.cardVerificationCode ||
    !req.body.cardNumber ||
    !req.body.SSN
  )
    return res.status(400).send("Bad Request");
  ...
  const clientCardvalidityDate = date.format(
    new Date(req.body.cardValidityDate),
    "YYYY/MM"
  );
  // Checking Card Validity against Expiry Date
  if (currentDate > clientCardvalidityDate)
    return res.status(400).send("Your Card is Expired");
  // Searching for the card
  const clientBankAccount = await Bank.findOne({
  ...
  const realValidityDate = date.format(
    new Date(clientBankAccount.cardValidityDate),
    "YYYY/MM"
  );
  console.log("realValidityDate " + realValidityDate);
  if (realValidityDate !== clientCardvalidityDate)
    return res.status(404).send("Card is unauthorized because of the date");
  ...
  try {
    ...
    await Client.updateOne(
      { _id: client._id },
      { $set: { bankAccountNumber: bankAccountNumber, activated: true } }
    );
...
router.put("/activateDependentAccount", async (req, res) => {
  if (
    ...
    !req.body.parentSSN ||
    !req.body.dependentUserName ||
    !req.body.dependentEmail
  )
    return res.status(400).send("BAD REQUEST");

  const parent = await Client.findOne({
    SSN: req.body.parentSSN,
  ...
  if (parent.isDependent || !parent.activated)
    return res.status(400).send("Your Parent is Underaged Or Not Activated");
...
router.put("/confirmingDependent", async (req, res) => {
  if (!req.body.email || !req.body.dependentEmail)
    return res.status(400).send("BAD Request");
...
router.post("/requestRide", async (req, res) => {
  ...
  if (!client)
    return res.status(400).send("Client was not found or is NOT AVAILABLE");
  ...
  if (!bike)
    return res
      .status(404)
      .send("Bike was not found or It was found NOT AVAILABLE at the moment ");
  ...
  if (bike.state === "Available") {
    try {
      ...
      const notification = new Notification({
        ...
        viewed: false,
        message:
          "Your Ride Request is Confirmed.  Use the PIN Code to Unlock your Bike.  PIN:" +
          randomPIN
      });
